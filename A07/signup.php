...
if (isset($_POST['btnSign'])) {
  ...
  if ($password == $confirmPassword) {
    if (!empty($username) && !empty($email) && !empty($password) && !empty($phoneNumber)) {
      ...
      if (executeQuery($infoQuery)) {
        $userInfoId = mysqli_insert_id($conn);

        $userQuery = "INSERT INTO users (userName, email, password, phoneNumber, userInfoId) VALUES ('$username', '$email', '$password', '$phoneNumber', '$userInfoId')";
        ...
        <div class="card p-5">
          <form method="POST" onsubmit="return validatePassword()">
            ...
            <div class="mb-3">
              ...
              <input type="password" name="confirmPassword" class="form-control frm-sign" id="confirmPassword" required>
            </div>
            <div class="mb-3">
              <label for="phoneNumber" class="form-label">Phone Number</label>
              <input type="text" name="phoneNumber" class="form-control frm-sign" id="phoneNumber" required>
            </div>
            <div class="mb-3">
              <button name="btnSign" type="submit" class="btn btn-sign">Sign Up</button>
            </div>
          </form>
